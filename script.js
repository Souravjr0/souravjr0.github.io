...
async function loadGitHubRepos() {
  ...
  try {
    ...
    if (!Array.isArray(repos) || repos.length === 0) {
      grid.innerHTML = `<p class="text-gray-400 text-center py-12">No public repos found yet.</p>`;
      return;
    }
    ...
    repos.forEach((repo) => {
      ...
      card.innerHTML = `
        <div class="flex justify-between items-start mb-4">
          <h3 class="text-2xl font-semibold text-white group-hover:text-cyan-400">${repo.name}</h3>
          <span class="text-xs px-3 py-1 bg-black rounded-2xl text-cyan-400">${repo.language || '—'}</span>
        </div>
        <p class="text-gray-400 line-clamp-3 mb-6">${repo.description || 'No description provided.'}</p>
        ...
        <div class="flex items-center justify-between text-sm">
          ...
          <a href="${repo.html_url}" target="_blank"
             class="text-cyan-400 hover:text-white flex items-center gap-1 text-sm">
            View repo →
          </a>
        </div>
      `;
    ...
    console.error(error);
    if (loading) {
      loading.innerHTML = '❌ Could not load repos. Please try again later.';
    }
  }
}
