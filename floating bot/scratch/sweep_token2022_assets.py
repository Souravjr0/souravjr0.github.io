...
logger = logging.getLogger("SweepToken2022")
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30.0) as http:
        for mint_str in mints:
            ...
            # Find the active ATA by trying both programs
            active_ata = None
            active_program = None
            active_balance = 0.0
            active_raw_balance = 0
            
            for prog_str, prog_name in programs:
                prog_pubkey = Pubkey.from_string(prog_str)
                ata = derive_ata(keypair.pubkey(), mint_pubkey, prog_pubkey)
                ...
                try:
                    ...
                    if "result" in resp_json:
                        ...
                        logger.info(f"  [{prog_name}] Found ATA: {ata} | Balance: {ui_amount} ({raw_amount} raw)")
                        active_ata = ata
                        active_program = prog_pubkey
                        active_balance = ui_amount
                        active_raw_balance = raw_amount
                        break
                except Exception:
                    pass
            
            if not active_ata:
                logger.warning(f"No active ATA found for mint {mint_str} under standard or Token2022 programs.")
                continue
                
            # If we have a balance, sell it!
            if active_raw_balance > 0:
                logger.info(f"Selling {active_balance} tokens via PumpPortal...")
                sell_payload = {
                    "publicKey": str(keypair.pubkey()),
                    "action": "sell",
                    "mint": mint_str,
                    "amount": active_raw_balance,
            ...
            # Close the account to reclaim the rent!
            logger.info(f"Closing ATA {active_ata} via Program {active_program} to reclaim 0.00204 SOL rent...")
            try:
