...
async def get_initial_states():
    ...
    payload = {
        ...
        "params": [
            [RAYDIUM_CLMM_POOL, ORCA_WHIRLPOOL],
            {"encoding": "base64"}
        ]
    }
...
async def main():
    ...
    print(f"Initial Raydium sqrt_price: {r_state['sqrt_price']}, liquidity: {r_state['liquidity']}")
    print(f"Initial Orca sqrt_price: {o_state['sqrt_price']}, liquidity: {o_state['liquidity']}")
            ...
            async with asyncio.timeout(30):
                async for message in ws:
                    ...
                    if data_len == 1544:
                        ...
                        print(f"[Raydium #{r_updates}] Price diff: {diff_price}, Liquidity diff: {diff_liq}, raw_len: {data_len}")
                        ...
                        print(f"[Orca #{o_updates}] Price diff: {diff_price}, Liquidity diff: {diff_liq}, raw_len: {data_len}")
                        if diff_price != 0 or diff_liq != 0:
                            o_state = new_o
                    else:
