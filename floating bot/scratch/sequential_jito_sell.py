...
logger = logging.getLogger("SequentialJitoSell")
...
async def get_sol_balance(http, rpc_url, pubkey) -> int:
...
async def sequential_sell():
    ...
    # Ordered by expected output (Hakimi first, then $SMACK, then DOG)
    mints_to_sell = [
    ...
    async with httpx.AsyncClient(timeout=30.0) as http:
        for mint, label in mints_to_sell:
            logger.info(f"\n==================================================")
            logger.info(f"STARTING LIQUIDATION FOR {label}")
            logger.info(f"==================================================")
            
            # Fetch current balance
            sol_bal = await get_sol_balance(http, rpc_url, keypair.pubkey())
            logger.info(f"Current SOL Balance: {sol_bal / 1e9:.6f} SOL ({sol_bal} lamports)")
            ...
            if ui_amount <= 0:
                logger.warning(f"Zero balance for mint {mint}. Reclaiming rent directly if active...")
            ...
            # Calculate dynamic safe tip based on current balance
            # Use 75% of our current balance as the Jito tip to absolutely guarantee landing!
            # Minimum 20k lamports, maximum 2M lamports (0.002 SOL)
            tip_lamports = max(20000, min(2000000, int(sol_bal * 0.75)))
            ...
            logger.info(f"Submitting Jito Bundle with Swap + {tip_lamports} Lamport Tip (~{tip_lamports/1e9:.6f} SOL) to {tip_account}...")
            ...
            try:
                ...
                if jito_resp.status_code == 200:
                    ...
                    if bundle_uuid:
                        ...
                        logger.info("Waiting 15 seconds for bundle to land...")
                        ...
                        if v_accounts:
                            ...
                            if v_amount == 0:
                                logger.info(f"[SUCCESS] Jito bundle landed on-chain!")
                                await reclaim_rent_direct(http, rpc_url, keypair, ata_pubkey)
                            else:
                                logger.warning(f"[FAILED] Balance still positive: {v_amount}. Discarded by block engine validators.")
                        else:
...
async def reclaim_rent_direct(http, rpc_url, keypair, ata_pubkey):
    ...
    try:
        ...
        if tx_sig:
            ...
            # Sleep 3 seconds to let block confirmations catch up
