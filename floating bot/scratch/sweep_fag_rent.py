...
logger = logging.getLogger("SweepFagRent")
...
async def main():
    ...
    # FAG Mint & Token2022 Program
    mint_str = "7y9DQxC3kFqtYzJX14MApsQCfdHraorBkmSbE38Epump"
    mint_pubkey = Pubkey.from_string(mint_str)
    token2022_program = Pubkey.from_string("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb")
    
    # Derived Token2022 ATA
    ata = Pubkey.from_string("e4Wn3mhu7KS5VZF2i7YZEYpCbCF4pAZrDpqbWvLtoZq")
    ...
    async with httpx.AsyncClient(timeout=30.0) as http:
        ...
        if raw_amount <= 0:
            logger.info("Account is already empty or burned!")
            return
        ...
        logger.info("Broadcasting burn + close transaction...")
            ...
            try:
                ...
                if sig:
                    ...
                    if status_val and status_val.get("err") is None:
                        logger.info("Transaction confirmed! FAG account successfully burned and closed! SOL rent reclaimed!")
                        break
                    else:
                        err_msg = status_val.get("err") if status_val else "Pending/Dropped"
                        logger.warning(f"Failed to confirm reclaim: {err_msg}. Retrying...")
                else:
